...
`src/domain/**` (soon `@lifeplanner/core`) MUST be pure & deterministic:
...
- Randomness uses the **seeded RNG** in `src/domain/seed.ts` (`hashSeed`), never `Math.random()`.
- Enforced automatically: an ESLint `no-restricted-syntax` rule on `src/domain/**` (see `eslint.config.mjs`) + the PostToolUse `check-edit` hook runs ESLint on every edited `src/**` file.
