...
## Goal model (current: two-tier + loose) &六个地基 (Phases 16/17)
- **Goal model is TWO-TIER**: `Goal.kind: "long"|"short"` + `parentGoalId`. Long = direction/identity, grows a life-tree branch (pathId), area-bump on complete, progress = roll-up of its short children + own units. Short = time-boxed (startDate/endDate), parented to a long OR standalone (parentGoalId null), no branch. `Subgoal` removed (migrated → short goals). Habits bounded to owning goal's [startDate,endDate] window (daily.habitInWindow; loose habit/no-goal = always). Migration `migrateGoals.ts` handles 3 input shapes (legacy-flat / nested / two-tier), idempotent, id-preserving, every-input-id-reachable invariant; runs from normalizeLoadedTree.
- **Loose (goal-less) items**: `LifeTree.tasks[]` / `LifeTree.habits[]` (daily routines like 上班). goalTree adapters return `goal: Goal | null`; loose render under a neutral "无目标" group (IconList). Created via calendar 「＋任务」/「＋日常」 or the home **quick-add bar** (`domain/quickParse.ts` parses 今天/明天/周三/每天/每周三/7点/14:00/#tag → loose task/habit; AppContext.quickAdd).
- **AI**: `/api/plan-short-goal` + `domain/planShort.ts` (localPlanShort spreads a short goal's tasks across [start,end] at sensible cadence + assigns weekly habit weekdays; offline fallback) → ShortGoalCard 「AI 规划这一段」 preview→apply. Existing arrange-day/decompose unchanged.
- **Reminders/PWA**: `domain/reminders.ts` dueReminders(overdue/today/upcomingSoon) → home 「今日提醒」; Notification API fires while app open (ReminderScheduler, dedupe per-day in localStorage); `public/sw.js` offline shell (network-first nav, bypasses /api & /_next, **registered production-only** so dev/HMR unaffected); installable PWA (manifest.ts). True background push = needs VAPID+backend (pending).
- **Calendar import (read-only)**: `domain/ics.ts` parser + `/api/ics` proxy (https-only, size/time cap, rate-limited); `LifeTree.calendarFeeds[]` (url subscription persisted; file uploads store inline events); read-only muted overlay on Month/Day (no tz math, no RRULE expand in v1). 2-way OAuth = pending.
- **Cloud sync**: Supabase magic-link auth + SupabaseRepository, BEHIND `isSupabaseCloudEnabled` (presence of NEXT_PUBLIC_SUPABASE_URL/ANON_KEY). Default OFF = unchanged localStorage. On sign-in: migrateLocalToCloud (one-time) then async cloud load/debounced-save; load-fail → local fallback + notice. UNTESTED vs real Supabase. Setup: docs/supabase-setup.md. @supabase/supabase-js installed.
- **Theme**: Apple-white minimal (light tokens, system SF, no serif); line icons everywhere (no emoji); tree/prediction on a `.lp-media-dark` panel.
