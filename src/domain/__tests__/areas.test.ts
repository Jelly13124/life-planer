...
function buildTree() {
  ...
  // career: one active long goal with a daily-repeat action (habit)
  let gCareer = createGoal({ area: "career", horizon: "long", title: "升到高级工程师", why: "涨薪" }, NOW);
  gCareer = setGoalActions(gCareer, ["每天刷题"]);
  gCareer = setActionRepeat(gCareer, gCareer.actions[0].id, "daily");
  t = upsertGoal(t, gCareer);

  // career: one active short goal (no habits)
  const gCareerShort = createGoal({ area: "career", horizon: "short", title: "完成项目A", why: "年终奖" }, NOW);
  t = upsertGoal(t, gCareerShort);

  // health: one active goal, no habits
  const gHealth = createGoal({ area: "health", horizon: "short", title: "减脂5斤", why: "精力好" }, NOW);
  t = upsertGoal(t, gHealth);

  // growth: done goal — must NOT appear in active list
  const gGrowthDone = createGoal({ area: "growth", horizon: "short", title: "读完《思考快与慢》", why: "" }, NOW);
  t = upsertGoal(t, { ...gGrowthDone, status: "done" });

  // wealth / relationships: zero active goals

  return { tree: t, gCareer, gCareerShort, gHealth };
}
...
describe("areaSummaries", () => {
  ...
  it("defaults score to 50 when profile.areas key is missing", () => {
    // Build a tree whose profile.areas is missing a key by casting
    const { tree } = buildTree();
  ...
  it("groups active goals correctly by area", () => {
    ...
    expect(goalIds).toContain(gCareer.id);
    expect(goalIds).toContain(gCareerShort.id);
    expect(careerS.goals).toHaveLength(2);

    const healthS = summaries.find((s) => s.area === "health")!;
    expect(healthS.goals.map((g) => g.id)).toContain(gHealth.id);
  ...
  it("counts habits (recurring actions) per area", () => {
    ...
    const summaries = areaSummaries(tree);
    const careerS = summaries.find((s) => s.area === "career")!;
    // The career goal has 1 daily-repeat action → 1 habit
    expect(careerS.habitCount).toBe(1);
    // health goal has no repeat action → 0
    const healthS = summaries.find((s) => s.area === "health")!;
