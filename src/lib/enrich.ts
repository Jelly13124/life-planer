...
function buildUserPrompt(input: EnrichInput): string {
  ...
  lines.push(
    "- 真实感优先于励志：绝大多数人生是「有得有失、部分实现、代价明显」，不是逆袭爽文。整条 mood 分布要贴近现实——不要一路 high，至少 1/3 的时刻是 mid 或 low。结局（最后 1-2 个 node 与 summary）不必是高光大团圆：可以是「达成一部分、失去一部分」「稳住了但没起飞」「换了方向重新开始」「和解但仍有遗憾」这类真实收束。别把每条路都写成成功学；宁可克制、留白，也不要过度乐观。",
  );
  if (input.kind === "choice") {
    lines.push("");
    lines.push(
      "评估这条路的【现实可行度】feasibility(0-100 整数)：衡量从主角真实起点（技能/资源/积蓄/约束/当前阶段）到这条路要求之间的差距——对他有多够得着。要校准、别都打高分：起点弱却要大跨越→低；顺势而为/已有基础→高。校准锚点：多数认真尝试但需要跨越的路落在 30–55；只有起点强且顺势而为才 > 65；起点弱 + 大跨越应 < 35；别动辄给 60+，现实里大多数选择没那么容易。注意：可行度评估的是这条路本身的现实门槛，不随乐观/保守走向变化（同一选择的不同走向可行度应基本一致）。再给一句 ≤20 字的依据 feasibilityNote（如『有设计功底+已起号，但变现门槛高』）。",
    );
  }
  lines.push("");
